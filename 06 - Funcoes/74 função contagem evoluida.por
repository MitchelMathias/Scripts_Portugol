// -----------------------------------------------
// Arquivo: 74 função contagem evoluida
// Tema: Funções e modularização
// Resumo: 74 função contagem evoluida
// -----------------------------------------------
programa
{
	inclua biblioteca Util --> u  // Inclui a biblioteca Util para funÃ§Ãµes auxiliares
	
	// FunÃ§Ã£o para realizar uma contagem de 'i' atÃ© 'f' com passo 'p', ajustando se 'p' for negativo
	funcao contagem(inteiro i, inteiro f, inteiro p){
		inteiro cont = i  // Declara a variÃ¡vel 'cont' e a inicializa com o valor de 'i'
		
		se (p < 0){  // Verifica se o passo 'p' Ã© negativo
			p *= -1  // Transforma o passo em positivo
		}
		
		// Se o inÃ­cio for menor ou igual ao fim, faz contagem crescente
		se ( i <= f){
			escreva("----- contando de ", i , " atÃ© ", f, " -----\n")
			// Loop que incrementa o contador de 'i' atÃ© 'f' com passo 'p'
		...
		// SenÃ£o, faz contagem decrescente
		senao{
			escreva("----- contando de ", i , " atÃ© ", f, " -----\n")
			// Loop que decrementa o contador de 'i' atÃ© 'f' com passo 'p'
			para(cont ; cont >= f; cont-=p ){
				escreva(cont, "--> ")  // Exibe o valor atual da contagem seguido de uma seta
				u.aguarde(250)  // Pausa de 250 ms entre cada nÃºmero exibido
			}
	...
	// FunÃ§Ã£o principal que realiza vÃ¡rias contagens diferentes
	funcao inicio()
	{
